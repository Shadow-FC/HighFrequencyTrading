...
class StockPool(object):
    ...
    def read_data(self, pool_name: str = 'StockPool1') -> pd.DataFrame:
    ...
    # ST股标识
    def ST(self, data: pd.Series) -> pd.Series(bool):
        """
        对ST,*ST股进行标记
        合格股票标记为True
        """
    ...
    def liquidity(self,
        ...
        amount_th: 单位为千
        """
        amount_mean = data.groupby(KN.STOCK_ID.value,
                                   group_keys=False).rolling(days, min_periods=1).mean()
        # 空值不参与计算
    ...
    def suspension(self,
        ...
        J = data.isnull()
        suspension_days = J.groupby(KN.STOCK_ID.value, group_keys=False).rolling(days, min_periods=days).sum()
        suspension_days = suspension_days.groupby(KN.STOCK_ID.value, group_keys=False).bfill()
        res = pd.DataFrame(dict(J1=suspension_days <= frequency,
                                J2=~ J))
    ...
    def StockPoolZD(self) -> pd.Series(bool):
        """
        1.剔除ST股：是ST为True
        ...
        result_path = os.path.join(self.local_path, func_name + '.csv')

        if os.path.exists(result_path):
            stock_judge = pd.read_csv(result_path,
                                      index_col=[KN.TRADE_DATE.value, KN.STOCK_ID.value])[func_name]
        else:
            # read data
            ...
            data_input['judge1'] = self.ST(data_input[PVN.ISST.value])
            ...
            data_input['judge3'] = self.established(data=data_input[PVN.LIST_DAYS_NUM.value])

            print(f"{dt.datetime.now().strftime('%X')}: Weed out stock illiquidity")
            data_input['judge4'] = self.liquidity(data_input[PVN.AMOUNT.value])

            print(f"{dt.datetime.now().strftime('%X')}: Weed out Suspension stock")
            data_input['judge5'] = self.suspension(data_input[PVN.AMOUNT.value])

            print(f"{dt.datetime.now().strftime('%X')}: Weed out liq_mv less than 0.02 stock")
            data_input['judge6'] = self.liq_mv(data_input[PVN.LIQ_MV.value])

            print(f"{dt.datetime.now().strftime('%X')}: Weed out science and technology innovation board stock")
            data_input['judge7'] = self.list_board(data_input[PVN.LIST_BOARD.value])
            ...
            # to_csv
            stock_judge.to_csv(result_path, index=True, header=True)

        # switch judge to label
        stock_pool = stock_judge.groupby(KN.STOCK_ID.value, group_keys=False).shift(1).fillna(True)
        return stock_pool[stock_pool]
